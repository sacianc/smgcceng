...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Repositories
{
        ...
        where T : TableEntity, new()
    {
        ...
        /// <param name="filter">Filter to the result.</param>
        /// <returns>All data entities.</returns>
        public async Task<IEnumerable<T>> GetCustomWithFilterAsync(string filter)
        {
            var messageAcknowledgeFilter = TableQuery.GenerateFilterConditionForBool(
                nameof(SentNotificationData.SentNotificationDataEntity.MessageAcknowledged),
                QueryComparisons.Equal,
                true);

            var combinedFilter = this.CombineFilters(filter, messageAcknowledgeFilter);
            var query = new TableQuery<T>().Where(combinedFilter);
